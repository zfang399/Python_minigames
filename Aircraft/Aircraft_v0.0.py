...
def GameSingle():
	...
	#stage settings
	stage_now=1
	stage_req=[100,1000,3000,6000,10000,15000,25000,40000,60000]
	...
	while True:
		...
		if now_time - scg_last > scg_cd:
			updated_g=True
			scg_last=now_time
			scg_side.append(randint(10,stage_now*10))
		...
		for bul in abul_pos:
			...
			if bul[1]<0:
				...
				count=0
				while count<len(scg_pos):
					if bul[1]+abul_speed>scg_pos[count][1] and bul[1]<scg_pos[count][1]+scg_side[count]:
		...
		while count<len(dang_pos):
			if dang_pos[count][1]+dang_side[count]>640:
				...
				pygame.draw.rect(screen,dang_color,pygame.Rect(dang_pos[count][0],dang_pos[count][1],dang_side[count],dang_side[count]))
			count+=1
